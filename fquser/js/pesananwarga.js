...
function getNamaPerumahan() {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: '../fquser/getdata/get_nama_perumahan.php',
            method: 'GET',
...
function getKategoriJasa() {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: '../fquser/getdata/get_kategori_jasa.php', // Ganti dengan path yang sesuai
            method: 'GET',
            success: function(data) {
                resolve(data);
            },
                ...
                case 1:
                    getNamaPerumahan().then(namaPerumahan => {
                        return getKategoriJasa().then(kategoriJasa => {
                            // Pastikan kategoriJasa adalah array
                            if (!Array.isArray(kategoriJasa)) {
                                throw new Error('Data kategori jasa tidak valid');
                            }
                    
                            // Mulai membangun konten
                            let optionsHTML = '<option>--Pilih Layanan--</option>';
                            
                            // Menambahkan opsi kategori jasa ke dalam select
                            kategoriJasa.forEach(kategori => {
                                optionsHTML += '<option value="' + kategori + '">' + kategori + '</option>';
                            });
                    
                            // Mengatur konten HTML
                            content.innerHTML = '<div class="containerlayanan">' +
                                '<h1>Pilih Layanan</h1>' +
                                '<p>Perumahan Anda: ' + namaPerumahan + '</p>' + // Menampilkan nama perumahan
                                ...
                                optionsHTML + // Menambahkan opsi kategori jasa
                                '</select>' +
                                '<br>' +
                                '<button onclick="goToStep(2)">Selanjutnya</button>' + // Tombol Selanjutnya
                                '</div>';
                        });
                    }).catch(err => {
